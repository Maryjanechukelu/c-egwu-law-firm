...
export default function ArticleClient({ article, metadata, author }) {
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Article Header */}
      <section className="sm:pt-20 pt-16 pb-16 bg-slate-50 border-b border-slate-200">
        <div className="container mx-auto px-4 md:px-8 max-w-4xl text-center">
          ...
          <div className="flex items-center justify-center gap-6 text-slate-500 text-sm border-t border-slate-200 pt-8 inline-block mx-auto">
            ...
            <span className="flex items-center gap-2">
              <Clock size={14} /> {metadata.readTime}
            </span>
          </div>
        </div>
      </section>
        ...
        {author && (
          <div className="flex items-center gap-4 mb-12 pb-8 border-b border-slate-100">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Principal Counsel</p>
            </div>
          </div>
        )}
            ...
            ? article.content.split("\n\n").map((paragraph, idx) => {
                if (paragraph.startsWith("## "))
                  return (
                    <h2
                      key={idx}
                      className="text-3xl text-slate-900 mt-12 mb-6"
                    >
                      {paragraph.replace("## ", "")}
                    </h2>
                  );
                if (paragraph.startsWith("- "))
                  return (
                    <ul
                      key={idx}
                      className="list-disc pl-5 space-y-2 mb-6 text-slate-600"
                    >
                      {paragraph.split("\n").map((item, i) => (
        ...
        <div className="mt-20 pt-10 border-t border-slate-200 flex justify-between items-center">
          ...
          <div className="flex gap-4">
            <Button
              variant="outline"
              size="icon"
              className="rounded-full border-slate-200 text-slate-500 hover:text-amber-600"
